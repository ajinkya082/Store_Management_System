...
const SalesPage: React.FC = () => {
  ...
  const handleCreateSale = () => {
    ...
    const customer = mockCustomers.find(c => c.id === parseInt(selectedCustomer));
    const newSale: Sale = {
        id: `SALE-${String(sales.length + 1).padStart(3, '0')}`,
        customerName: customer?.name || 'Unknown',
        products: cart.map(item => ({ productId: item.id, productName: item.name, quantity: item.quantity, price: item.price })),
        total: total,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md">
          ...
          <div className="overflow-auto max-h-[500px]">
            <ul>
              {sales.map(sale => (
                <li key={sale.id} className="border-b dark:border-slate-700 py-3">
                  <div className="flex justify-between font-semibold">
                    <span>{sale.id}</span>
                    <span className="text-green-500">${sale.total.toFixed(2)}</span>
                  </div>
                  <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    <span>{sale.customerName}</span> - <span>{sale.date}</span>
                  </div>
                </li>
              ))}
            </ul>
